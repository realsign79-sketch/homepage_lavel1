...
export function GET() {
  ...
  const lines = [
    ...
    `- 주소: ${getAddress(t)}${has(t.postalCode) ? ` (우편번호 ${t.postalCode})` : ""}`,
    ...(has(t.jibunAddress) ? [`- 지번: ${t.jibunAddress}`] : []),
    ...(typeof t.latitude === "number" && typeof t.longitude === "number" ? [`- 좌표: ${t.latitude}, ${t.longitude}`] : []),
